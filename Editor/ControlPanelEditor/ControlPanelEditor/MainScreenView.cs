...
public class MainScreenView : GraphView
{
    ...
    private void СreateСonnections(BehavioureTree tree, NodeBase node)
    {
        //получить детей ноды
    ...
    private NodeView FindNodeView(NodeBase node) => GetNodeByGuid(node.guid) as NodeView;
    ...
    //создать ноду
    public NodeBase CreateNode(System.Type type, Vector2 position = default)
    {
        NodeBase node = treeModel.CreateNode(type);
    ...
    public void CreateNodeView(NodeBase node)
    {
